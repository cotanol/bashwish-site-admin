...
const ProjectTables = ({ data: propsData }: { data?: any }) => {
  // Use venues as projects
  const projectTable =
    propsData?.products?.slice(0, 7).map((venue: any, index: number) => ({
      id: index + 1,
      projectTitle: venue.productName,
      leader: 'Admin',
      team: [],
      progressValue: venue.status === 'Published' ? 100 : 50,
      hours: `${venue.qty || 0} kids`,
      progressColor: venue.status === 'Published' ? 'success' : 'warning',
      totalTask: `${venue.qty || 0}/100`,
      budgetSpent: `$${venue.price || 0}`,
      chipLabel: venue.status,
      chipColor: venue.status === 'Published' ? 'success' : 'warning'
    })) || []

  // States
  const [rowSelection, setRowSelection] = useState({})
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [tableData, setTableData] = useState(projectTable)
  ...
  const table = useReactTable({
    data: tableData as ProjectTableRowType[],
    columns,
    filterFns: {
      fuzzy: fuzzyFilter
    },
