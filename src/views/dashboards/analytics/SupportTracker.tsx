...
const SupportTracker = ({ data }: Props) => {
  // Hooks
  const theme = useTheme()

  // Calculate activity rate (active venues / total venues)
  const activityRate =
    data.overview.totalVenues > 0 ? Math.round((data.overview.activeVenues / data.overview.totalVenues) * 100) : 0

  // Generate dynamic data
  const venueData: DataType[] = [
    {
      title: 'Active Venues',
      subtitle: data.overview.activeVenues.toString(),
      avatarColor: 'success',
      avatarIcon: 'tabler-check'
    },
    {
      title: 'Featured',
      subtitle: data.overview.featuredVenues.toString(),
      avatarColor: 'warning',
      avatarIcon: 'tabler-star'
    },
    {
      title: 'Avg. Rating',
      subtitle: `${data.overview.averageRating} ⭐`,
      avatarColor: 'info',
      ...
      <CardContent className='flex flex-col sm:flex-row items-center justify-between gap-7'>
        <div className='flex flex-col gap-6 is-full sm:is-[unset]'>
          <div className='flex flex-col'>
            <Typography variant='h2'>{data.overview.totalVenues}</Typography>
        ...
        <AppReactApexCharts type='radialBar' height={350} width='100%' series={[activityRate]} options={options} />
      </CardContent>
    </Card>
  )
}
