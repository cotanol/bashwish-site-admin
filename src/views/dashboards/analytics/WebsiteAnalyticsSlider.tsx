...
const Slides = ({ data }: Props) => {
  // Generate dynamic data based on real analytics
  const slidesData: DataType[] = [
    {
      title: 'Venue Traffic',
      img: '/images/cards/graphic-illustration-1.png',
      details: {
        'Total Venues': data.overview.totalVenues.toString(),
        Active: data.overview.activeVenues.toString(),
        Featured: data.overview.featuredVenues.toString(),
        'Conv. Rate': `${data.overview.conversionRate}%`
      }
    },
    {
      ...
      details: {
        Searches: data.overview.totalSearches.toString(),
        Clicks: data.overview.totalClicks.toString(),
        Conversions: data.overview.conversions.toString(),
        'Email Leads': data.overview.totalEmailCaptures.toString()
      }
    },
    {
      title: 'Performance',
      ...
      details: {
        'Click Rate': `${((data.overview.totalClicks / Math.max(data.overview.totalSearches, 1)) * 100).toFixed(0)}%`,
        'Lead Conv.': `${((data.overview.totalEmailCaptures / Math.max(data.overview.totalSearches, 1)) * 100).toFixed(0)}%`,
        'Venue Conv.': `${((data.overview.conversions / Math.max(data.overview.totalClicks, 1)) * 100).toFixed(0)}%`,
        Success: `${data.overview.conversionRate}%`
      }
    }
  ]
  ...
  return (
    <>
      {slidesData.map((slide: DataType, index: number) => {
        return (
          <div key={index} className={classnames('keen-slider__slide p-6 pbe-3 is-full')}>
            ...
            <Typography variant='subtitle2' className='mbe-3 text-[var(--mui-palette-common-white)]'>
              Total {slide.details['Conv. Rate'] || slide.details.Success || data.overview.conversionRate}% Conversion
              Rate
            </Typography>
...
const WebsiteAnalyticsSlider = ({ data }: Props) => {
  // States
